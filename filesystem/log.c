...
void log_msg(const char *format, ...)
{
    ...
    vfprintf(OS_DATA->logfile, format, ap);
...
void log_fuse_context(struct fuse_context *context)
{
    ...
    log_struct(((struct os_state *)context->private_data), logfile, %08x, );
    log_struct(((struct os_state *)context->private_data), rootdir, %s, );
